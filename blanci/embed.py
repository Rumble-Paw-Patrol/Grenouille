...
def select_recordings(
    ...
    if peak_hours:
        hours = (pd.to_datetime(df["start_utc"], utc=True) + pd.Timedelta(hours=utc_offset_h)).dt.hour
...
def embed_recordings(
    ...
    for (dataset, site, month), group in recordings.groupby(["dataset", "site", "month"]):
        ...
        for rec in group.itertuples():
            ...
            con.executemany(
                "INSERT OR IGNORE INTO windows (window_id, recording_id, offset_s, dur_s) VALUES (?, ?, ?, ?)",
                [(wid, rec.recording_id, offset, dur) for wid, (offset, dur) in zip(ids, windows)],
            )
            metas.append(pd.DataFrame({"window_id": ids, "recording_id": rec.recording_id,
            ...
            if report.recordings % progress_every == 0:
                print(f"  {report.recordings} enregistrements, {report.windows_per_s:.1f} fenêtres/s",
                      flush=True)
        flush()
...
def register_encoder(con: sqlite3.Connection, encoder: Encoder, hop_s: float, report: EmbedReport) -> None:
    params: dict[str, Any] = {
        ...
        "last_run": asdict(report) | {"windows_per_s": report.windows_per_s,
                                      "realtime_factor": report.realtime_factor},
    }
    con.execute(
        "INSERT INTO models (model_id, kind, name, version, params_json, created_at) "
        "VALUES (?, 'encoder', ?, ?, ?, ?) ON CONFLICT(model_id) DO UPDATE SET params_json = excluded.params_json",
